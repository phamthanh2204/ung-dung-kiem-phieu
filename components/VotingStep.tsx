...
const VotingStep: React.FC<VotingStepProps> = ({ candidates, ballotCount, votes, onVote, onFinish }) => {
  return (
    <div className="p-4 md:p-8 mx-auto">
      <h2 className="text-2xl font-bold text-green-400 mb-6 text-center">Bước 2: Bỏ phiếu</h2>
      <p className="text-center text-gray-400 mb-6">Nhấp vào ô tương ứng để bỏ phiếu. Mỗi hàng là một lá phiếu, có thể chọn nhiều ứng viên.</p>
          ...
          <tbody>
            {Array.from({ length: ballotCount }, (_, i) => (
              <tr key={i} className="bg-gray-800 border-b border-gray-700 hover:bg-gray-600">
                ...
                {candidates.map(candidate => {
                  const isVoted = votes[i]?.includes(candidate);
                  return (
      ...
      <div className="mt-8 text-center">
        <button
          onClick={onFinish}
          className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-lg shadow-lg transform hover:scale-105 transition duration-300 ease-in-out"
        >
